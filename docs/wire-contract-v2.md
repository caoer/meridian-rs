...
**Notation:** this document carries no wikilinks; the `[[…]]` sequences inside the §0.3 fenced fixture are data bytes — they are what `resolve` (§4.5) and `links` (§4.6) operate on — and are covered by the computed hashes.
...
1. The interface ruling: five verbs (`toc/cat/edit/append/resolve`), sections-as-files, Edit-exact old/new scoped to a section, **requests never require revs / receipts always return them**, no byte offsets in the interface, mandatoriness = Go policy ratchet, resolve = walk plane.
...
3. The ratified foundations: Starlark (§11.4) and the DuckDB organ (§10.4).
...
mdfs_config.yaml                     (non-md — structurally outside the domain, §12)
```
...
- `mdfs_config.yaml` v0 declares an empty custom ignore list; v1 adds `drafts/**` (§12.3). Its own bytes are never hashed (non-md, §12.1).

Timeline: **S0** →(E3 edit)→ **S1** (`plan.md` 139 B, receipts 249 B) →(E4 append)→ **S2** (`plan.md` 150 B, receipts 474 B). Roots:

| State | Root (full width, never truncated) |
|---|---|
...
The wire has exactly five nouns. Four carry forward from the frozen `crates/wire` vocabulary (`Path`, `Span`, `NodeRev`, `Root` — `crates/wire/src/lib.rs`, newtypes, serde-only); the fifth — **Delta** — is born here, at contract birth.
...
| `Root` | `"b3:" + 64 hex`, full width | algorithm+domain prefixed; prefix bumps on domain-rule change (§12.3); never truncated |
| `Delta` | the change-fact object, §7 | node-grain at birth; frozen shape; replay ≡ live |
...
**One hash family:** BLAKE3-256 everywhere (rev, file_rev, leaf, interior, root). The repo deliberately left the algorithm open (`model` crate doc: "algorithm is a rung-2 wire amendment"); **this schema is that amendment** — it decides blake3, consistent with the `wire::Root` doc example (`"b3:…"`) and node-rev-merkle-spec §1.
...
| `NodeRev`/`Root` | opaque tokens; algorithm swap = domain-prefix bump (§12.3), engine mechanism unchanged for consumers |
...
The strict grammar is THE fleet grammar; the Obsidian algebra is a syntactically disjoint, read-only compatibility input. Our extensions (revs, roots, occurrence index, `mdfs_config.yaml`) never appear inside the walk grammar; `[[##`/`[[^^` search syntaxes are UI, out of scope.
...
{"id":1,"ok":true,"body":{"proto":1,"server":"meridian-sidecar/2.0",
  "caps":["toc","cat","extract","resolve","resolve.content","links","links.require_root",
          "splice","splice.if_node_rev","splice.if_root","splice.dry","splice.receipt",
          "splice.verdicts","root","diff","sub"],
  "root":"b3:74162a12ff0b323b52be37359cf5144fcc254ecf8801958402514a763829b5e9"}}
```

`caps` is the complete set — no version sniffing, ever. Field-only amendments ship as dotted `op.field` strings. `root` in the hello body is optional (the engine may not have walked yet); when present it is the first ambient root.
...
| `root` | 3 | integrity fact |
| `diff` | 3 (reserved shape, frozen now) | replay (§7) |
| `sub` | 5 (reserved shape, frozen now) | delta transport (§7) |

The v1 §6.4 `Guard{root,path}` reserved op is **dropped**: the integrity surface is `root` + `splice.if_root` + `diff` — the mined commit-guard idiom gets its wire story without a second guard grammar: one construct, one grammar.
...
toc is the complete write kit: hpath + `node_rev` per section, anchors with their revs when present, frontmatter keys. The header `root` makes the commit-guard idiom ambient — read a toc, later pass `if_root`. `content_span` is served for interface use (heading-preserving display) but mints nothing (§1).
...
### §4.3 extract — the frozen §5 surface, stands

`{"op":"extract","path":…,"kinds":[…]}` stands as frozen (`crates/wire` §5): full node objects, 11-variant kind enum whose declaration order is the sort-tiebreak ordinal, per-kind `info`, `text_prefix_16b` (implemented + tested in `crates/wire-map`), total node order (span.start asc, span.end desc, kind ordinal). One decision this schema makes: **an unknown value in `kinds` is `bad_request{"unknown_kinds":[…]}`, loud** — the strict-server evolution law applied to values, killing the typo-silently-returns-nothing trap. This diverges from v1's "not an error"; decided once, here.
...
Batch laws: batch-only, ONE response shape; all targets and guards resolve against the **pre-batch** state; targets must be disjoint (`bad_request{"overlap":…}` otherwise); the batch commits atomically through one reparse — a post-apply parse that would lose containment refuses `would_corrupt{"lost":[hpath…]}`. `dry:true` runs everything except disk: same response shape, `root_after:null`, no receipt written.
...
Note the guardless request: legal at the wire forever — the mined call record shows zero organic rev use, and guards stay optional at the wire by design. Whether a scope *requires* `if_node_rev`/`if_root`/`actor` is the Go policy ratchet (§5.3), never wire law.
...
{"id":80,"ok":true,"body":{
 "as_of_root":"b3:83b4ba591c0291d9f2a05428cac38e5820858fbb9c47720ab352344ddccc8f68",
 "live_root":"b3:83b4ba591c0291d9f2a05428cac38e5820858fbb9c47720ab352344ddccc8f68",
 "changes_seq":2,
 "files":{"notes/plan.md":{
...
Shape mirrors the app's `resolvedLinks`/`unresolvedLinks` — per-edge counts; dangling refs first-class. `path` absent → whole-corpus edge map. Opt-in `require_root` → `stale_view` refusal (§10.2).

### §4.7 root and diff — the integrity rung

```json
{"id":90,"op":"root"}
{"id":90,"ok":true,"body":{
 "root":"b3:83b4ba591c0291d9f2a05428cac38e5820858fbb9c47720ab352344ddccc8f68","seq":2}}
```

`diff` is reserved AT the integrity rung with its shape frozen now — the compound front door:
...
Replay ≡ live (§7.3). A root range outside the retained history → `root_unknown` → full resync (honest bound: §13.5). `sub` (rung 5) is a reserved shape — `{"op":"sub","from_seq":N}` → ok, then Notification frames each carrying one Delta batch; only the transport is deferred, the noun it carries is frozen here — rung 4/5 defers only transport.

### §4.8 The five-verb interface, mapped

| Verb | Wire op | Notes |
|---|---|---|
| `md toc F` | `toc` | rows `level · span · rev · hpath` — the map, revs riding free |
| `md cat F --sec A B` | `cat` | `--sec` segments → hpath array verbatim |
| `md edit F --sec A B <<<old/new>>>` | `splice` with one `match` edit | `--if 9d3e…` → `if_node_rev`; receipt address defaulted by the client library, overridable |
| `md append F --sec A B` | `splice` with one `put{at:"end"}` edit | |
| `md resolve F '[[x#H]]'` | `resolve` | interface strips `[[…|alias]]`; wire takes raw linktext — one grammar |
...
`if_root` compares against the current workspace root and is checked FIRST — world-grain, cheapest, fails the whole batch (`root_mismatch{expected,actual,changed}` → re-plan); then per-edit `if_node_rev` (node-grain, `cas_mismatch{expected,actual}` → refresh). Merkle-spec §7 semantics carried; Rust computes hashes, Go only compares.
...
The wire is permissive forever: unguarded, actor-less, receipt-less splices are legal wire frames. Requiredness — "shared scopes need `if_node_rev`", "this tree needs `actor`", "receipts mandatory under `results/`" — is the Go daemon's per-scope policy ratchet, tightening AFTER the tooling earns usage — the geography law. Precedent machinery for per-scope layered config (two-axis resolution, provenance labels, loud degradation) already exists in the fleet's Go daemon — cited as the ratchet's natural home, not as wire law.

**Scope grammar (bound here):** a ratchet scope is expressed in the fleet vocabulary and no other — a `Path`-set selector (path globs, config data that never rides the wire) plus, where a scope names nodes, strict-plane refs per §2.1. No second address grammar exists in policy config, and the §11.3 pack manifest carries no scope field by design: packs bind rules to the world model; the ratchet binds requiredness to scopes, Go-side. (Fix-at-freeze, §18 row 1.)

## §6 Receipts — outcome as fact

### §6.1 The law

A write intent that names a receipt gives its address as `receipt:{path, anchor}` on the splice request — receipts are per-request, never a wire requirement (unguarded, actor-less, receipt-less splices are legal frames, §5.3; whether a scope *requires* one is the Go ratchet, §5.3, not wire law). When named, the engine appends the receipt entry to that md file **in the same batch commit** as the content edit — one wire exchange, one reparse, ONE root advance covering both files. Receipts are ordinary markdown inside the hash domain (composes with the md-only domain law, §12): addressable, cat-able, resolvable via `#^anchor`, hashed into the root like everything else.

The armed response (§4.4) and the receipt entry carry the same facts: op, target identities, rev transitions, `root_before`, actor, now, request id. Facts about what was **armed** — never "delivered", never "succeeded downstream" (outcome-as-fact; delivery is Go's business).

### §6.2 No-self-rooting law (honest limit)

A receipt **cannot contain the root it produces**: `root_after` covers the receipt file's bytes, so writing `root_after` into the receipt is structurally impossible (the hash would have to contain itself). Receipts therefore carry `root_before`; `root_after` rides the wire response and the Delta. This is stated as a limit, not worked around.

### §6.3 The worked receipt (E3's, exact bytes appended to `receipts/2026-07-18.md`)
...
One list-item block per batch; the block anchor (`^r-000042`, Form A at end-of-block — dialect-legal) is the receipt's address; its computed node facts: span `[26,248]`, `node_rev 639a2dca46f6fcc8` (block span excludes the final terminator, v1 law). `Goals>Q3` here is display text inside a default template, not a second address grammar (§6.4). E4's receipt, exact bytes appended at S2 (span `[249,473]`, rev `c912d4578883f288`):
...
## §7 The Delta noun — the fifth noun, born frozen

### §7.1 Shape (frozen at contract birth)

One Delta = one batch = one root advance. E3's delta, every value computed:
...
### §10.4 The rung-5 view organ — DuckDB, ratified

DuckDB is the rung-5 organ under the measured conditions — wire-only face, rebuild-is-the-update, view-never-store, root-stamped `view.duckdb` (disposable) + `changes.duckdb` (append-only, `batches(seq, root_before, root_after, ts)` minimum schema — note: this contract's Delta already carries exactly those fields). **No wire element changes under the ratification:** every view-shaped element declares the triple + `stale_view` regardless of engine, the R5 line and rebuild-is-the-update stand as contract law, and the wire stays engine-agnostic by construction — no op, field, or error names DuckDB.
...
Findings, never decisions: severity ∈ {error, warn, info}; whether an `error` *blocks* is Go policy, not engine behavior. `budget_exceeded` is a finding in this array. The shape is `crates/policy`'s `Violation{rule, severity, path, span, node_rev, hpath, message}` verbatim — the typed stub already matches this schema.

### §11.2 WHEN/HOW partition (validation-enforced)

A rule's WHEN sees world-model facts only — nodes, revs, spans, links, the given `now`/`actor`; its HOW is opaque data the engine never interprets (escalation, notification, remediation — Go's business). A pack whose WHEN references anything outside the fact vocabulary fails compile (`UnsupportedVocab`, existing `policy::CompileError` variant).
...
## §12 The root domain — md-only + `mdfs_config.yaml`

### §12.1 The domain

The hash domain rule, verbatim: "mdfs_config.yaml define the custom ignore list. mdfs only work for md file. it does not hash any other file." The hash domain = markdown files ONLY, minus the ignore list. `.git/index`, locks, `.obsidian` artifacts never self-invalidate a guard — dissolved by construction. `mdfs_config.yaml` is not md ⇒ structurally outside its own domain.

**Proposed DEFAULT ignore list — one rule:** any path with a dot-prefixed segment is ignored (`.github/README.md`, `.obsidian/**`, `.trash/**`). One rule, zero enumeration, covers every observed dot-dir case; the gitignore-semantics machinery — patterns, `!` re-includes, `domain-of-root ≡ domain-of-service` — is the harvest for the *custom* list's mechanics in `mdfs_config.yaml`.

**Hash domain ⊂ addressable domain:** ignored md files remain fully addressable — `toc`/`cat`/`splice` on `.github/README.md` all work; its bytes simply never enter the root. Worked counterfactual — a wrong ignore implementation cannot pass this fixture pair:

| Domain | Root (computed) |
|---|---|
...
### §12.2 Interior encoding (merkle spec §4, carried verbatim)

Leaf = blake3(whole raw file), full 32 B. Interior = blake3 over children sorted by raw name bytes, each `varint(len(name)) ‖ name ‖ type_byte(0x00 file / 0x01 dir) ‖ hash32` (varint = LEB128; a single byte for lengths <128); empty dirs pruned; the workspace root's own name never hashed; modes/symlinks/unicode-normalization per spec §4/§9 (root never crosses hosts). Invalid-UTF-8 md files leaf-hash and participate but serve no spans — integrity coverage ⊥ span service. All roots in this document were computed by this exact encoding.

### §12.3 Domain-rule changes bump the prefix

An ignore-list (or algorithm) change re-defines the domain, so the prefix advances: `b3:` → `b3a:` → `b3b:` … (base-26 suffix after `z` — `b3aa:`). The engine learns the current suffix at cold start by reading a **`version` field in `mdfs_config.yaml`** — the same file that defines the domain (§12.1), so the domain definition and its prefix travel together; it never counts config history (that would collide with the no-storage law, §14, the same way a persisted `seq` would, §7.1). Worked, computed: at S2 with `drafts/tmp.md` present —
...
The v1 hex **equals R2's hex** (same surviving file set — computed, `bump_hex_equals_R2: true`) yet the tokens can never compare equal: a foreign cursor holding a `b3:` root cannot silently match a `b3a:` world. That is the entire point — chosen over the ring-wipe alternative because receipts (§6) persist roots durably in md files, where silent cross-domain comparison would otherwise be a real hazard. Live cursors are 32-byte disposable state; re-root recovers (geography law).


## §13 Threat and limit register (the honesty standard, throughout)

1. **16-hex rev truncation:** ≈2^32 birthday work on attacker-fed content forges a rev collision. The mitigation is the trusted-local boundary — this wire serves local, trusted workspaces; any "adversary-proof" claim is dead. Full-width `root` and `file_rev`-over-whole-file are the honest escalation ladder for wider guarantees.
2. **Staleness:** no lag bounds, ever (§10.1). `as_of_root` may trail `live_root` at any moment; `require_root` is refusal, not synchronization.
...
4. **Conformance unknowns are pack-pinned, never asserted:** stage-1 duplicate-basename tie-break, embed depth-cap constant, app version drift — frozen as `obsidian-compat@1.12.7` answers at default settings, re-generated per app version via the live oracle (`obsidian eval`). The pack and its regeneration operating manual live in-repo at `crates/testsuite/data/gt/obsidian-compat/` (see its `PROVENANCE.md`). Hand-frozen resolution fixtures are dead.
5. **Root-history ring bound 256:** older ranges answer `root_unknown` → full resync. Re-derive, never wrong data.
6. **Crash window in two-file batches** (§6.5): content-without-receipt is possible; loud via lint, recovered via re-root.
7. **No-self-rooting** (§6.2): receipts structurally cannot carry their own `root_after`.
...
| `crates/wire` | all four noun newtypes; frozen `Toc`/`Extract`/`Hello`; `Node` + kind ordinal; `ErrorBody` typed extras; strict/tolerant obligations | +`Delta` noun; +ops `cat`/`links`/`root`/`diff`/`sub` and reshaped `resolve`/`splice` via rung-freezing amendments; `ErrorCode` grows the §8 splits; `not_found` retired; §6.4 `Guard` op dropped |
...
| `crates/testsuite` / GT | consolidated test binary, frozen GT pack + provenance | GT regenerated from THIS contract: lane pack demoted; resolution GT = app-generated `obsidian-compat@1.12.7`; every deviation row ships a fixture the deviated-from dialect FAILS (the discrimination law); `wsfix/` values join as fixtures |
...
- **Ground truth is regenerated from this contract, not hand-frozen.** GT is regenerated from the contract; the lane pack is demoted; the discrimination law holds; all resolution GT is app-oracle `obsidian-compat@1.12.7` (§14, §13.4).
...
- **The hash domain is md-only, config-declared, with a counterfactual root pair worked.** md-only domain + `mdfs_config.yaml`; one-rule default ignore; the hash domain is a subset of the addressable domain; a counterfactual root pair is computed; a domain-rule change bumps the root prefix to `b3a:` (§12).
...
| `pipe` | Dead; staged-commit value re-expressed as `dry` + atomic batch + `if_root` plan guard (§4.4, §5.1) |
...
**Cross-cutting mined facts honored:** the zero-organic-rev reality is served by a permissive wire + ambient revs + a Go ratchet (§4.4, §5.3); invisible in-process consumers become wire clients (the MCP row above; an existing engine-linked consumer is transition-tolerated only, ruled out by panel law); zero-use flags are not carried; unknown request fields reject loudly (§3.2) — with the corrected heading-read count honored via `resolve.content`.
...
| Worked-value honesty (A4) | tool statement (header) + `results/wire-contract-v2-verify.py` + every value in §§0.3–§12 computed; DRAFT/FROZEN banner top |
| Usage-pattern coverage (A5) | §16 (the pattern index + cross-cutting facts) |
| Delta noun (A6) | §7 (delta at birth, frozen shape, `diff` reserved §4.7, replay ≡ live §7.3, grain ruled node-grain §7.4) |
...
| View topology (A10) | §10 (triple, `stale_view` worked, no lag bounds, R5 structural, DuckDB organ ratified §10.4) |
...
| Skill doc + HTML page (A13, A14) | Downstream deliverables (skill doc, HTML page) — bound to this contract + the design-stance; not claimed here |

**Rulings attest:** the formerly open rows are ruled and folded — node-grain deltas (§7.4), Starlark (§11.4), the DuckDB organ (§10.4), the single block-id charset (§2.4); deviations and waivers are consolidated in §18. **Tool attest:** blake3 = `uvx --from blake3 python3` over `results/wire-contract-v2-verify.py`, which rebuilds the fixture from this document's own §0.3 bytes; offsets = Python byte math over those bytes; zero invented values. **Execution honesty (A6 restated):** what has been executed for this document is the value recomputation only; replay ≡ live (§7.3) and the conformance packs are stated law, demonstrated on computed frames, and not yet exercised by any executable test — those tests are impl-rung deliverables (§18 row 5).
...
| # | Item | Disposition |
|---|---|---|
| 1 | Policy-scope grammar was unbound to the strict plane (ratchet scopes lived as Go-side prose; the pack manifest has no scope field) | **FIXED** — §5.3 now binds scope grammar: `Path`-set selectors + strict-plane refs (§2.1), no second grammar; the manifest's scope-field absence is declared deliberate |
| 2 | The repo's reserved `root_mismatch` shape (`crates/wire` §6.5 reserved-codes note) carries extra fields `expected/actual/scope/changed`; this contract ships `{expected,actual,changed}` — the `scope` drop was unflagged | **WAIVED, declared** — the only root guard is world-grain `if_root` (§5.1); no scoped-root construct exists for `scope` to describe. If a scoped root guard ever arrives by amendment, `scope` returns with it. The skill's ERRORS.md documents the same three-field shape — consistent |
...
| 4 | Two silent rebinds vs the ruled failure-class table plus one dropped code | **FIXED, declared** — §8 now declares all three deltas with rationale: `root_mismatch`→`resync`, `unsupported_proto`→`respawn`, `bad_id` folded into `bad_request` + `id:null`/`id_raw`. Behavior-preserving |

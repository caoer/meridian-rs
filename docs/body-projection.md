...
The sql face is the DuckDB projection operators query, not agent core. This spec rules how a page's body text becomes rows on that face: how the text is cut into chunks, what the `body` relation serves, and how the cache stores each chunk once. No wire op changes (§7.4).

Normative for `body` in both sql lanes (the ephemeral `:memory:` build, the `sql.duckdb` cache). Also law: `node-rev-merkle-spec.md` (node_rev, content span), `laws.md` § crate charters, `view::store` module docs (cache protocol).

## §1 The grain

Questions arrive as *which section says X*, so that is the grain. One row per **chunk**: a section's own text (children excluded) or the document **preamble** (text before its first heading). Heading lines are in **no** chunk (`section.heading` serves them). Whole-document text is not a relation; the out-of-engine `mrd read` (measured 0.52 s corpus-wide) serves it.

## §2 The chunk law

- **Section chunk:** from content start — the byte after the heading line's terminator, by `model`'s content-span law — to the first child's span start, or to the section's span end when childless. Only a heading closes a section, and any heading opens a child, a sibling, or closes the section. Exclusive content is therefore one contiguous run: every body byte lands in exactly one chunk.
...
Both lanes serve the same face — the columns a caller sees. The DDL is the contract (`crates/view/src/schema.rs` mirrors it):
...
The shape is `task`'s: its own seq space, a nullable governing section, an advisory `hpath`. The population is the same: rows that usually belong to a section but legally may not. `node_rev` is the owning section's CAS token, not a chunk hash; there is no chunk splice door.

## §4 The cache lane

The cache is append-only; `hist.*` is its only storage. Each re-projection appends the document's full row set at the next generation, so a plain text column would re-store a document's whole text on every edit. The protocol splits identity from content instead:
...
Only `hist.body_text` rows dedup, across generations and paths: an edit appends text rows only for chunks new to the file's history. Orphaned `body_text` rows compact only at rebuild-and-swap. The pin protocol is unchanged: one file, one transaction, one fingerprint. `as_of_fingerprint` covers body bytes, so no second freshness witness exists. `base_fold` has one only because `.base` bytes are outside the fingerprint.
...
- **Second file (ATTACH)** — rejected: breaks the single-file single-transaction pin invariant (the file is always at exactly one fingerprint; §4); DuckDB has no cross-file transaction.

...
One row per **chunk**: a section's own text (children excluded) or the document **preamble** (text before its first heading). Heading lines are in **no** chunk (`section.heading` serves them). Whole-document text is not a relation; `mrd read` (0.52 s corpus-wide) serves it.

## §2 The chunk law

- **Section chunk:** content start (the byte after the heading line's terminator; `model`'s content-span law) to the first child's span start, or the section's span end when childless. Only a heading closes a section, so every body byte lands in exactly one chunk.
...
The shape is `task`'s: own seq space, nullable governing section, advisory hpath. `node_rev` is the owning section's CAS token, not a chunk hash: it guards the `splice` a search hit leads to; there is no chunk splice door.

## §4 The cache lane

The cache is append-only (`hist.*` is its only storage), so identity and content are split:
...
- **Second file (ATTACH)** — rejected: breaks the single-file single-transaction pin invariant ("the file is always at exactly one fingerprint"); DuckDB has no cross-file transaction.
- **Lazy body materialization** — rejected: forks the pin into a second freshness dimension inside the fingerprint's domain, where no second witness exists (§4).
- **FTS index in the cache file** — rejected: `PRAGMA create_fts_index` / `drop_fts_index` per pin is edit-shaped work in a never-vacuumed file, and the measured 3.36–5.83 s rebuild would ride the daemon's per-save append path. Search on the face is LIKE/regexp over `body.text`; a caller may build a per-call FTS index in the rollback lane. A persistent side-artifact index is a separate future design, taken only if per-call friction shows.
...
- **Cache append:** one staged anti-join insert; text grows only by new chunk bytes, not by documents touched.
- **`:memory:` lane, per query:** ~51k chunk slices (~34 MB transient); `mrd sql` stays a slow operator tool by design.
...
2. **No phantoms:** no preamble, no preamble row; a frontmatter-only doc emits zero body rows.
...
Landing adds relation `body` (with `hist.body` / `hist.body_text` in the catalog), new `information_schema` rows, and `SCHEMA_VERSION` 7. Conformance re-records ride the same landing; downstream clients' teaching surfaces follow the served face.
...
Search hit to guarded write in one row: `SELECT path, hpath, node_rev FROM body WHERE text LIKE '%sub body%'` returns the address and CAS token a `splice` needs.

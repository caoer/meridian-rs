...
- **mtime+size leaf cache** — rejected for v1: its lie window (same mtime+size, different bytes) buys warm-rebuild speed we don't need once the tree is memory-resident and event-updated (§6). Cold start eats the 2–5s honestly. *Under the resident tree (§6):* the memo is `StatKey`-keyed, and the lie window this bullet fears is closed by law, not by abstinence — the §6.2 watermark + stable-read protocol (a same-instant same-size in-place edit still refuses; the hermetic gate row).
...
serving window. Sharding is taken now, not deferred; the cutover is a priced
protocol, paid once, behind the pre-cutover code blockers §4.2.5 names.
...
- **Why it retires:** re-folding a directory re-encodes ALL its children, so a
  ...
  shard later") is rejected as a priced less-worse, and the cutover is taken
  while no scoped tokens exist in the wild.

### 4.2 Merkle law 2 — the fixed-256 radix child map (the new hash-law version)

Each directory's child
list is a canonical radix map with fanout fixed at 256 — one child slot
("bucket") per byte value — so one change re-hashes a bounded
number of vertices, never every sibling. The requirement is a bounded-fanout
authenticated map, canonical (history never affects the result), instantiated
at the fixed-256 shape. The cost
...
flat-100k acceptance gate publishes this as operation counts: per-commit
unrelated-member stat/read/hash counters = 0, plus the
flat-100k directory case with its same-vertex-count discipline. Width for
calibration: a measured live corpus's maximum directory width was 428 — the
cutover is cheap at that width and a cliff later.

#### 4.2.5 Versioning — one law current, typed retirement

- The hash-law version is a dimension of its own, ORTHOGONAL to the
  workspace-domain version: the
...
- No dual-hash serving window: maintaining two trees to keep old tokens valid
  ...
  workspace — never on every read; `sub` re-baselines at a labeled epoch
  boundary, never a silent chain break.
- **The cutover boundary, stated as law.** The cutover runs behind three
  pre-cutover code blockers — cursor identity, this section's second
  version dimension, durable cutover authority state — then
  walks `OLD_SERVING → DRAINING → no-return boundary → NEW_BUILDING →
  NEW_COMMITTED → reopen`. The no-return boundary is crossed by ONE
  durable act: making the cutover record's `NEW_BUILDING` state
...
- **The downgrade fence never activates.** The fence mechanism
  (`crates/fs/src/fence.rs`) is dormant — `activate` has no production
  caller — and is never activated on downgrade grounds: its threat model
  is empty (no old-binary users; a leftover old binary is deleted, not
  fenced), so a downgrade fence is not a cutover blocker. Old-BINARY
  fencing is not an axis of this law; old-TOKEN refusal
  (`fingerprint_version_retired`, above) is. A non-downgrade reason for
  the fence to activate would be a new design question — no such reason
  exists.
- **The shadow-build fallback is NOT BUILT.** The one-time per-workspace
  pause is accepted, so the bounded NON-SERVING shadow — whose only
  purpose would be to shrink that pause — is not implemented: no second
  tree is built beside the serving one, even as preparation. Acceptance
  MEASURES and RECORDS the real pause; it does not gate on a budget. The
  bullet above bans a dual-hash SERVING window; this one says the
  non-serving BUILD does not exist either — together they leave an
  implementer no shadow to reach for.
...
mints or compares a vertex token (nothing below a path node holds a scope).
...
- The subtree fold (`mrk2.dir` at a folder scope) remains the conservative
  fallback and the explicit directory-premise form. Consistency law: every
  set premise — pattern root, selector root, sql
...
The shape follows the Bazel unique-segment precedent. Law 1 ignored
...
> **Frontmatter row receipt** (regenerated through the engine). The
...
> family — `wire-contract.md` §18 row 3 (waived, declared); a leaf-law
> reading (`[0,19)`) is not what the engine serves. Regeneration (fixture
> workspace = this section's two files, byte-exact):
>
> ```
...
> rows and the fingerprint match this section's pinned values.)
...
(All values computed by a reference implementation of this spec, blake3-256; the generator is `node-rev-merkle-spec.assets/worked-example-gen.go`, with `node-rev-merkle-spec.assets/go.mod`, and is the fixture seed for the test suite. The interior/fingerprint values above are LAW 1 values; `node_rev` and leaf values are law-independent and survive the cutover.)
...
Real values (this section's byte layout is the law; these numbers are its
receipt). Generated by the law-2
...
The alternative — derive-on-demand: no tree in memory, a full stat sweep +
full fold per currency pass, no persistence, cold rebuild on every daemon
start — pays two flock-held full-corpus reads per guarded write (~1.5 s of
corpus reading, measured) to keep 32 bytes, and makes every world guard
root-grain. The resident tree is what replaces that cost.
...
- **Own writes update the tree synchronously.** A commit knows the exact
  ...
  `root_after` / `fingerprint_after` derives from this
...
- **Memory:** ~10 MB at a 30k-member corpus (ESTIMATED, never measured — flagged);
  full fold from already-known leaves 12.1 ms (measured);
...
3. Reads **open without following links**; an identity check (fstat) runs
   ...
   re-read. *(The no-follow walk is a directory-fd `openat(O_NOFOLLOW)` per
   component **from the workspace root down** — every component BELOW the
   root refuses a symlink; the root itself opens as the caller named it,
   symlinks in its own prefix followed. The root is the trust anchor, not a
   member: every production root is canonical at bind
   (`workspace::canonicalize`), so the prefix-follow is a no-op there, while
   a fixture root under macOS's `/var` → `/private/var` `$TMPDIR` is not
   refused for its own prefix — a walk that began at `/` would refuse the
   whole tree on a default-`$TMPDIR` mac.)*
...
instruments on every path; the acceptance row is the "same-tick
...
- **Stamps are instance-bound:** ring seq
...
- **The engine owns its senses.** One kernel file watcher (FSEvents on macOS,
  ...
  that feeds the tree; and a client daemon is a client — the engine's
  freshness must not depend on a client's uptime.
...
- **The rescan ladder — every cause NAMED, throttled:** kernel event overflow
  → mark-all-dirty → the next pass is the full stat sweep (160 ms
...
- **Idle re-check — SUSPICIOUS-ONLY, NO TIMER.** The engine
...
Requirement 2 — "the engine knows, it does not
...
- **Restart replay** (by gap class): where a QUALIFYING
...
- **Where no qualifying journal covers the gap** (today, every process
  ...
  The pass is: one stat per member as the floor — the 160 ms figure
  at 29.7k members measures THIS STAT PASS ALONE — plus the watermark law's
...
- **Two questions, two instruments.** A checkpoint is an observation record
...
**Motive, measured.** §6.1/§6.4 rule the vouched currency answer ("O(dirty),
zero when quiet — never O(corpus)") and the write door rides it
(`Registry::door_observation`). This section states WHICH standing
consumers the law binds — left unstated, every one of them keeps the
§6.2 extent-refresh floor unconditionally. Measured on a live corpus
(37.8k members): a daemon in that state burns ~66–85
...
timers, while the feed, the cookie, and the overlay sit unused on
the read side.

> **The law.** Every standing currency question in the daemon is answered by
> the workspace's ONE resident memo (§6.1)
...
> (the prewarm's standing license), and may never stand in for the content root: what
...
| prewarm quiet check | may this sweep be SKIPPED? | latency-only | O(1), no cookie: nothing pending after take-and-apply, memo `Trusted`, cached served fold == the engine's stamp. The `domain_stat_signature` walk survives ONLY where no live feed exists (`FeedSlot::Failed`), under the existing quiet backoff |
...
frame-parity gated; the alternative — one full corpus snapshot, every
member's BYTES, per emitted external batch — measured ~1/s under a stream of
foreign writes with a subscriber). When the
...
baseline) keeps its one full snapshot. The ring holds no private fold
memo — the shared memo is the one instrument on the watch plane too,
a plain mutex keeps the single-flight
...
**Motive, measured.** Like §6.7 this binds a stated law to its consumers:
§6.1 rules the maintenance law ("folds recompute lazily on demand —
maintenance cost follows change, never corpus size") and §4.2 puts the cost
law in the data structure. This section states that the law binds
DERIVING the served answer, not only deciding whether it may serve. A served
fold that rebuilds a FRESH radix tree over the whole leaf memo on every
recompute honours §4.2's per-change bound inside an instrument reconstructed
at O(corpus) per question, and an incremental corpus pass that deep-clones
every carried document pays again. Measured (37.8k members, 15 s sample
under 2 foreign writes/s): ~0.25 CPU-s of pure memory work per absorbed
change, ~26.6 CPU-s per 60 s — attributed to the two flat rebuilds (the
overlay fold and the rebuild's tail fold) and the carried-document clones.
...
Legality: every ADDRESSABLE node is a legal premise. Sufficiency (the
coverage law), field spellings (`scope`, `guards[]`, `scope_bytes`), and
requiredness geography are wire-side law — `wire-contract.md` §5.3–§5.4.
...
| workspace token (root scope — `if_fingerprint`) | the world | "is the WORLD still what I planned against?" | `fingerprint_mismatch` — resync, re-plan |
...
- **Raw-byte names are addressable** through the opaque canonical byte-path
  arm (`scope_bytes`: base64url raw segments; `wire-contract.md` §5.4 spells
  it) beside the UTF-8 `scope` convenience — mint and guard serve both. §9
  rules names integrity-covered AND addressable; the UTF-8 read-face serving
  limits stated there stand.
...
`wire-contract.md` §5.4). Bare is the world mint. Under `scoped-guards`:
...
skips narrower work (the two-guard ordering, generalized).

**What each layer never does:** the engine never decides *when* a guard is
**required** (host policy / geography — `wire-contract.md` §5.3); hosts never compute hashes (node_rev / fingerprint
...
- The scoped-premise fields (`scope`, `guards[]`, `scope_bytes`) are capability-advertised; a frozen v2 session using them un-negotiated refuses `bad_request` loudly, never silence. Field law: `wire-contract.md` §5.4.
...
- Routine writes route through the daemon: the CLI rides the daemon's resident tree over IPC, and `LOCK_EX` on `write.lock` is takeover/recovery only. The write plane's own law (lease, intents, parallel disjoint commits, the ruled plain-fsync class) is the authority contract's, not this spec's. Construction step 6's publication half — reservation algebra, checksummed `O_EXCL` intents, the one state owner's contiguous root chain — lives in `crates/wire-serve/src/publish.rs` (disk primitives in `crates/fs/src/intent.rs`); the lease half is `authority.rs`. The live write door keeps its interim flock until the cutover flips routing. The `apply_batch` pre-image verify is the in-process second-writer refusal under daemon routing (`docs/laws.md` Amendment — the one state owner).
- The effects lane (`run`, script-with-effects) is UNGUARDED by rule (the normative paragraph lives in `run-plane.md`) — but guard-free never means fold-invisible: every effects write rides the same write choke-point and maintains the resident tree (leaf update, chain refold, §6.2 watermark discipline). That is tree maintenance, not a guard.
- **File death mints no terminal hash — the death Delta is the record.** A guarded `remove` (`wire-contract.md` § A.3) unlinks the leaf; the next fold composes the tree without it under the current §4 encoding — removal is already in the diff shape (§0, "whole-subtree enumeration on add/remove") and no new hash law exists for it. Under law 2 this is the §4.2.2 delete rule: the child map re-canonicalizes as if the entry never existed. A rev is a function of bytes (§2); absent bytes mint nothing: the death's terminal facts are the removed file's LAST rev (`file_rev_before`, confirmed by the remove-what-you-read CAS) and the workspace fingerprint transition, both carried by the death Delta (`change:"deleted"`, `wire-contract.md` §7.1). No tombstone leaf, no on-disk marker — disk stays markdown only, and history past the ring re-derives to a world where the path is simply absent. The emptied path itself mints `absent` (§7) — a legal premise, not an error.
...
- **Name truthfulness:** there is no conversion — the content for merkle hashing is always truthful, and a conversion on the hash path is a flaw. A per-session display-layer conversion cannot drift, because within a defined snapshot it is two-way convertible with zero loss. Operationally: every fold — the workspace fingerprint, the exec-guard bracket folds, any derivation that claims to be the corpus root — carries the raw on-disk name bytes end to end. Conversion is legal only in display layers (error prose, listings), and there only where it is two-way convertible with zero loss for the servable set; an escaped rendering (`\xNN` for invalid bytes, `\\` for a literal backslash) is the display form for a name that has no UTF-8 spelling.
- **Non-UTF-8 names: hashed truthfully, addressable via `scope_bytes`, unservable on the UTF-8 read faces.** A domain member whose NAME is not valid UTF-8 still gets its leaf and enters the root with its exact name bytes (blake3 and the interior encoding need no UTF-8). The integrity plane serves it: mint and guard take the opaque raw-byte arm (`scope_bytes`, §7; field law `wire-contract.md` §5.4), so a premise can name it and coverage can count it. What remains true is the READ-face limit: wire paths on the serving surface are JSON strings (UTF-8 by construction), and no injective UTF-8 spelling exists that also keeps every valid name fixed — so such a member serves no spans, the serving snapshot (`DomainFiles`) carries only the UTF-8-named members, and a watch delta cannot name such a path; the frame's fingerprints stay truthful and the §6/§7 resync law covers what the delta cannot spell. Reachability: macOS refuses creating such names (errno 92); Linux is the reachable platform.
...
- **File/dir name collisions: both kinds hashed.** Law 2 hashes both kinds (§4.4 — nothing sits outside the fold), lints the build loudly, and refuses `scope_unresolved` at mint and guard on the colliding path. All the rulings above carry into law 2 unchanged: raw name bytes end to end are the trie's key bytes, byte-order sort is the slot and forest ordering (§4.2.2, §4.3.1).

...
| `wire-contract.md` §4.7 detect pre-check (`WorkspaceRing::detect`) | root moved since baseline? | latency-only + fallback clock | the same O(1) quiet check through the shared memo; the private fold memo serves `prime` and the miss path only. §6.6's poll survives: even under a quiet vouch the floor pre-check runs once per `DETECT_FLOOR_CADENCE` (30 s) — the push plane's backstop against silent capture loss. The subscriber thread that asks is itself event-driven: it parks between questions and is woken by a kernel event on its workspace (the feed rings the workspace's wake set), a frame recorded on its ring, its floor clock, its peer closing, or shutdown — never by a fixed tick. `DETECT_CADENCE` (250 ms) is the coalescing window across subscribers when a wake lands, and the wait interval only on a workspace with no live feed |

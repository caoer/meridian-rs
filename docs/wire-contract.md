...
| `transition_unrepresentable` | `target` (the offending edit's ref, §2.1 grammar) | a **pure insertion** (`at:"end"`) whose target still resolves and whose `node_rev` did not move — so not one inserted byte joined the node the edit names | aim the write inside the node (`at:"all"` re-supplying its content) or at the enclosing section, whose span contains the bytes you meant to add |

**Why `transition_unrepresentable` is a corruption family and not a reporting nicety (added 2026-08-09).** `node_rev` is defined as a function of the node's span bytes (node-rev-merkle-spec §2). A node whose span **excludes its line terminator** — every anchor block-leaf (§1) and every `fm_key` leaf (§4.4) — has its `at:"end"` insertion point sitting on that terminator, so `text` carrying a newline opens a fresh line the node never covers. Arming that commit states a transition that did not happen, and it silently disarms the caller's guard: `if_node_rev` then compares a value the write can never move, so two callers holding the same rev both write, both succeed, and neither is told. Repeated appends then land at one fixed offset and accumulate in REVERSE order.

**The family's scope is PURE INSERTIONS, and the bound is load-bearing.** A zero-width replaced region replaces none of the node's bytes, so the node can change only by adopting inserted ones — an unmoved rev is then a proof, not a heuristic, that the whole insertion landed outside the target. A `put{at:"all"}` is NOT judged here: it hands the node its own new content, so an unmoved rev means the caller wrote the same bytes back. *(Known residual, measured 2026-08-09 and deliberately left to its own ruling rather than folded in silently: an `at:"all"` rewrite whose `text` carries a trailing separator can still land that separator outside a terminator-excluding leaf, leaving `if_node_rev` unmovable on that node. It is not this family.)* The refusal is measured on the same one reparse the other families use — never inferred from the edit text.

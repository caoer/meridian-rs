...
**GOAL 1 — works-for-us (primary):** the ruled grammar (§2.1), the geography law (§5.3), and the rulings enumerated below, each naming the section that carries it:
...
4. Review checklist A1–A14; ratified convergence items: mint partition, 16-hex rev, md-only, two-stage resolve, app-oracle GT (ground truth).
...
Each item below is ruled, and the ruling is the contract text.
...
The wire has exactly five nouns: `Path`, `Span`, `NodeRev`, the content-hash type (`Root` in code; design noun **fingerprint**) from `crates/wire/src/lib.rs`, and **Delta**, which this contract defines.
...
**Span sub-laws:** a section node is heading-inclusive and newline-inclusive, with no trim; it runs to the next heading of level ≤ its own (else EOF) and contains the deeper headings inside it (node-rev-merkle-spec §5 pins stay live). A leaf block node excludes its final line terminator; an inline node includes its delimiters. A span splitting a multi-byte character refuses `bad_request` — guarantors: parser token discipline on reads, the reparse gate on writes (§15).
...
This contract requires one address grammar, carried by two planes. On the **mint plane**, a write target or strict read names a node exactly and the engine mints (issues) its rev token (§2.1). On the **walk plane**, `resolve` alone follows Obsidian's looser ref grammar (§2.2). The two are different verbs with different response types, and the walk response has no rev field, so a ref cannot arm a write: the mint partition.

### §2.1 The strict mint plane

Write targets and strict reads — `cat` and `splice` targets — name nodes by **exact name only**, in three forms (the same forms echoed in `toc` rows, receipts, deltas and verdicts):
...
The wire is NDJSON: one JSON object per line on the daemon's unix socket (§3.3). The socket carries frames only and logs go to stderr, so `echo '{"id":1,"op":"hello",…}' | nc -U "$SOCKET"` works by contract. Three frame types, classified by the **raw** `id` key: present → Request/Response (correlated); absent → Notification (§7 deltas ride here).
...
A connection opens with `hello`: the client names the proto it speaks and itself, and the server answers with its own identity and its caps.
...
- Warm workspace: the currency pass stays inline at every read, at the vouched grade — O(dirty) via the event feed's cookie proof, with an extent-refresh floor of O(domain) `stat`s and O(delta) parses on a named miss (`node-rev-merkle-spec.md` §6.7; `run-plane.md` § What an entry costs). Only the cold whole-corpus build leaves the read door.
...
Eleven ops. The § A.3 additions (`read`, `create`, `remove`) and § A.7 `script` land on top, not re-tabled; the five-verb interface maps 1:1 onto the original ten (§4.8). A read op belongs on the wire only when it feeds an action; orientation reads are dashboard-only. `Rung` names the position on §14's rung ladder.
...
**The CLI seam reads the `edits` value, not this envelope.** `mrd put <PATH>` takes the bare array under `"edits"` on stdin; `id`, `op` and `path` are argv's. A whole request object is refused, exit 2 before any engine contact: `the edits on stdin are not the §4.4 batch shape … stdin takes the BARE edits ARRAY, not the wire §4.4 request object: send the value of its "edits" field (id / op / path are argv's here)`.
...
| `remove{}` | **`fm_key` targets only**: strikes the key line from the frontmatter block, moving the key to R4's absent state (R4 = the three-state frontmatter law, absent ≠ null ≠ empty string, § A.6.2); no fields, no `text`. Absent key → `ref_not_found`; `hpath`/`anchor` target → `bad_request` (§ A.6.6) |
...
**Two ceilings, two audiences.** A limit that can refuse must be discoverable before it refuses. A caller who reads only the script budget under-builds the wire door: the first real batch ran 103 files, and a 64-file ceiling would have failed it on contact.
...
**Visibility is a lane property, not a set property.** A **lane** is the path a write takes to disk: the daemon's wire door, or an in-process caller (§3.3). `seq` and a Delta are minted only where a daemon serves the write; an in-process lane has no seq sink, answers `seq: 0` and mints no Delta, so its sealed set is invisible to every watch-plane consumer (catchup across it is diff-by-root, §4.7). The CLI is **not** that lane: every `mrd` write verb is a wire client on the daemon's door (`crates/mrd/src/write_ipc.rs`), so a `mrd put` set is numbered like any other wire set. A watched corpus writes sets through the daemon door.
...
`read`-as-link-oracle is the dominant read pattern of corpus tooling, and `links` is the fact op that serves it: corpus-wide, carrying the staleness triple (§10):
...
{"id":80,"ok":true,"body":{
 ...
 "changes_seq":2,"tree_instance":"1a0b32b824f.28f5.0",
 "files":{"notes/plan.md":{
...
`fingerprint` mints the workspace's content hash; `diff` replays the Deltas between two such hashes.

```json
{"id":90,"op":"fingerprint"}
{"id":90,"ok":true,"body":{
 "fingerprint":"b3:6e866e13b5e65ef9961c050f8a621cf1980b00ee293be650deef5f4dbc6823f0","seq":2,
 "tree_instance":"1a0b32b824f.28f5.0"}}
```

`seq` is the §10.1 counter in the minted token's tense — the retained frame that ended at this fingerprint, `0` when none does — and `tree_instance` is the epoch it is numbered in, the one value in this frame that is not a fold (minted per ring epoch: unix millis · pid · counter, §7.1). The pair is the cursor a `sub` resumption echoes, so a consumer can align a mint with the delta stream without subscribing first. A frozen v2 session's mint stays `{root, seq}`.

**The scoped mint arm (`scoped-guards` cap, §5.4).** Under the cap `fingerprint` takes `scope` (a `Path`, §1) or `scope_bytes` (base64url over the raw path bytes, for names the UTF-8 `Path` noun cannot carry), exactly one. Both absent: the root mint above — the frozen v2 key set, plus the v3 epoch identity. Both supplied: `bad_request`, a mint names one node (teaching text: §8.2's mint-pair text). The op mints the named node's token (root, folder, or file leaf), the one mint home for every premise the §5.4 guard family accepts. A lawful path with no node answers the reserved non-hex value `absent` (§5.6); an unlawful path refuses `scope_unresolved` (§5.6, §8). The response echoes the scope pair beside the token, `{fingerprint, seq, scope}` or `{fingerprint, seq, scope_bytes}`; `fingerprint: "absent"` is a legal body. Its `seq` is `0` and it names no epoch: the counter is a workspace-grain fact and this token names a node, so no retained frame ends at it (§10.1's `0` reading). No worked scoped hex is printed: the interior encoding moves under the width ruling (`node-rev-merkle-spec.md`).
...
- **Resumption:** `{"op":"sub","tree_instance":I,"from_seq":N}`; the instance is evaluated before any sequence compare. A dead instance (daemon restart, idle reap) refuses `root_unknown` with the diff-by-root remedy, sequence never consulted. A live-instance `from_seq` outside the retained ring also refuses `root_unknown`, as does one the retained chain cannot bridge contiguously — a change that went unnumbered while nobody watched and nobody read leaves a hole between two frames, and replaying across it would serve a stream that omits it (§7.1, §7.3).
...
A guard states a fact the caller expects to still hold; the engine re-derives that fact and compares it before any byte moves.
...
Scoped guards let one request carry several premises, each naming the path node it covers. The family rides behind the `scoped-guards` cap; every refusal below teaches per §8.2.
...
The § A.7 field wall (12 → 14: `guards`, `scope`) is this matrix; no fifteenth field exists. Sugar and list together are legal: the sugar desugars to one more premise. Per premise `{scope?, scope_bytes?, fingerprint}`: exactly one of `scope`/`scope_bytes`, or neither for the root premise; `fingerprint` is required and holds a token or `absent` (§5.6). Pair violations — `scope` or `scope_bytes` without `fingerprint`, both spellings in one premise, sugar `scope` without `if_fingerprint` — refuse `bad_request`, validated atomically at the door so hash and path cannot desync. The mint door has its own pair violation: both spellings on one `fingerprint` request refuses `bad_request` at §4.7 with its own text.
...
A fourth fact is the request, not the world. A premise value must be `absent` (§5.6) or a grammatical `Root`-family token (merkle-spec §4.2: `b3` + bijective-base-26 suffix + `:` + 64 lowercase hex); anything else refuses `bad_request` (recovery `fix`) at the premise rung, before any fold is compared. The refusal quotes the raw bytes debug-quoted, so a leading space shows as a byte. `fingerprint_mismatch` therefore claims one thing only: a well-formed token differed from the live fold. This wall is value grammar, never a permission plane, and touches no version family (§12.3).
...
- File `change ∈ {created, modified, deleted, renamed, unattested}`. `renamed` carries `from_path`; `unattested` (§ A.9's re-scope word) means the file left the attested set — the §12 hash domain — while its bytes remain on disk — v3-only, demoted to `deleted` for a frozen v2 session.
...
- External changes (a human editing in Obsidian) produce deltas with `actor`/`now` **absent** — the engine never invents identity or time. `seq` is assigned at detection, and **detection runs where someone is looking**: under a live subscription, and on any view-shaped read of an epoch that already has a baseline (§10.1). Adopting that baseline is a full corpus observation, which is a subscriber's cost to pay and not a reader's — a read door runs the cycle but never primes one. So numbering begins at an epoch's first subscribe, whose own first cycle adopts the world and describes nothing; a change made before that, or in an epoch nobody ever subscribed to, is described by no frame. No fact is lost and none is invented. So a stream is proved complete by its **roots**, each frame's `fingerprint_before` against the last `fingerprint_after`, never by `seq` arithmetic: the ring proves that chain contiguous from a cursor before it anchors a resumption, and a gap it cannot bridge refuses `fingerprint_unknown` → full resync (§4.7, §7.3) rather than replaying across it.
...
Every error frame carries `code` + `recovery` from the closed six-class enum. Each code is statically bound to exactly one class; a client that does not recognize a code dispatches on `recovery` alone.
...
The six classes ride **error frames**, where the daemon answered. A request whose answer never arrives — the op deadline expired, or the connection died in flight — is a **transport loss**, not a wire error. No `recovery` class exists, and for a `splice` **persistence is unknown**: the batch may have committed before the loss. A slow op is not a hung daemon.
...
Rust never reads a wall clock or env identity. `actor` (an opaque string) and `now` (an RFC 3339 string, format-validated, never generated) ride the wire as ordinary optional request fields on `splice`. They are recorded into receipts and Deltas, and they are rule-evaluation inputs (§11: temporal predicates read the *given* `now`). Absent inputs produce absent facts (worked: external-change deltas, §7.1). Whether a scope requires them is host policy (§5.3). Hosts may mint `actor` from their own session machinery; the engine never invents one.
...
`changes_seq` is the **retained frame that ended at `as_of_fingerprint`** — the counter in that tense, never the ring's tip published beside some other fingerprint. `0` says this answer aligns to no retained frame (a fresh epoch, an evicted window, a lane that numbers nothing), and its recovery is the cross-epoch one, diff-by-root (§4.7); it never says the corpus is quiet. A v3 response carries **`tree_instance`** beside the counter — the epoch the number is valid in, the identity a `sub` cursor must echo (§4.7) — because a ring is reborn with its workspace and a counter that reset is otherwise read as a corpus that went still; a frozen v2 session's `links` body stays the frozen key set. A view-shaped door runs one detection cycle before it answers (§7.1) — never a priming one — so once an epoch has a baseline the counter covers out-of-band change for a consumer who never subscribes, and in an epoch that has none it stays `0` rather than claiming a quiet the door did not observe.
...
A rules pack declares itself in one manifest:
...
Every reviewer-flagged debt is fixed or waived with a reason here, never silently. Rows 1–5 are the review fix list; rows 6–7 consolidate deviations declared in the body; rows 9–15 are measured against shipped artifacts and this document's arithmetic. Rows 1–7 record where this document departed from ruled law; rows 9–15 where a shipped artifact departed from this document, measured against the built binary. A deviation found without a row here is a contract bug (§15's assumption-audit law). **Declaring is not legislating:** the law a row names stays in force, unamended, and the row is the observation, never a licence.
...
| 12 | **The Delta counter reached no read door.** §7.1 laws one Delta per batch per fingerprint advance and §10.1's `changes_seq` is that counter; measured, every daemon-served write minted and retained its Delta — the CLI lane included, its verbs being wire clients (§4.4 lane clause, `crates/mrd/src/write_ipc.rs`) — while `links` answered `changes_seq: 0` and the §4.7 mint answered `seq: 0`, before and after. The row's first reading blamed the CLI lane for minting nothing; the lane mints, and the counter was published on the `sub` ack, the push frames and the writer's own splice response and nowhere else | **FIXED. §7.1 and §10.1 STAND UNAMENDED.** The view-shaped doors publish the retained counter at the fingerprint the answer was computed at, `tree_instance` beside it (§10.1), and run one detection cycle first, so a change no subscriber watched is numbered for the reader in any epoch a subscription has given a baseline (§7.1; a read door never primes one). A `0` now reads as *this answer aligns to no retained frame*, whose recovery is the cross-epoch one, diff-by-root (§4.7) — never as *nothing changed*. Gated by `crates/registry/tests/changes_seq_published.rs` |
...
| 15 | **A change nobody watched was never numbered.** §7.1 assigns an external change's `seq` at detection, and detection ran only under a live subscription: an out-of-band edit with no subscriber left the retained chain with a hole — measured, the next write's `fingerprint_before` is not the last frame's `fingerprint_after`, and no frame describes the interval — which the resumption anchor did not check, so a cursor across the hole replayed the numbered frames alone | **FIXED, and the residue DECLARED.** A view-shaped read runs a detection cycle (§10.1), so a workspace anyone reads is numbered with no subscriber, and the resumption anchor now proves the retained chain contiguous from the cursor before it anchors — a hole refuses `fingerprint_unknown` → full resync (§7.3's degrade-to-re-derive), never a replay across it. **The residue:** numbering begins at an epoch's first **subscribe**, whose first cycle adopts the world as its baseline; a read door runs the cycle but never primes it, that being a full corpus observation a read op's fold accounting does not carry. Until then reads number nothing and answer `0`. No fact is lost and none is invented; the interval is described by no frame, which is why a chain is proved by its roots and never by `seq` arithmetic |
...
widen: § A.6.3 still refuses a newline (D11, the multi-line-value refusal), so a
block-scalar value cannot
...
**Effects mode.** Two more request fields, so the field wall — the closed set of fields this op accepts (§3.2) — grows 9 → 11:
`effects:[…]` and `invocation`. Absent `effects` = the pure script above,
byte-identical. Present = the **live program** model (`run-plane.md`
§ Effects mode is normative):

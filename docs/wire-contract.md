...
**The `would_corrupt` families (amended 2026-08-09).** One code covers both post-reparse identity deaths, and the refusal body discriminates them with `family` — a caller dispatches on `family`, never on which extras happen to be present:
...
**The third width — what the shipped template actually WRITES (2026-08-09).** Both numbers above are about this document. The number a reader of a deviation ledger actually needs is the third one: replaying E3 under this section's own `actor` and `now`, so deterministic, the shipped default template writes **216 B node bytes** (**217 B line**, terminator included — the form the dogfood step reports, s15-70). Every width COMPARED in this section — 260/262, 222/224, and the 216 — is **node bytes, terminator excluded, per the leaf-block span law (§1)**; the line figure carries its own label because the two bases differ by exactly the terminator, and mixing them is what made this arithmetic wrong by 1 B until it was measured on both. So the shipped node is **6 B SHORT of the 222 B** the fixture's spans require, and **44 B short of the illustration above** — which decomposes to the byte: **6 B** for the omitted `id=` token (the CLI lane mints no wire request id, so the template writes none), **7 B** for spelling `root_before=` where §6.1's standing noun is `fingerprint_before=`, and **31 B** for joining the target as the pretty `Goals>Q3` this section says in its own words never to re-teach. 216 + 6 + 7 + 31 = 260. The 6 B shortfall is a **net against bytes this document does not print**, so it is not attributable to any one of the three — only the 44 B is decomposable. All three deviations are carded separately and are not fixed here; the consequence of the width is §18 row 10.

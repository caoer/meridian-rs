...
fn predicate_is_one_definition_shared_by_both_projections() {
    let bare = LockItem {
        ...
        to_root: None,
        root_refusal: None,
        // U21's cross-root MEASURED ABSENCE, added to `LockItem` on a branch
        // U9c never saw. A bare item has no absence to record, so `None` is the
        // bare spelling — the same reading every other field here takes.
        root_absence: None,
    };
    assert!(
        !bare.is_colourable(),

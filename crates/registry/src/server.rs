...
/// Spawn the reaper: park for `reap_interval`, then drop idle entries, then
/// evict over the resident budget ([`Registry::reap_to_budget`] — the idle
/// pass first, so the budget sweep only pays for what idleness alone could
/// not shed). The shutdown bell ends the park at once, however long the
/// interval, so the thread never ticks to stay responsive.
fn spawn_reaper(
    registry: Arc<Registry>,
    shutdown: Arc<Doorbell>,
...
/// The observation checkpoint's cadence: the small §6.5 checkpoint of every
/// warm workspace is coalesced once a minute — the saver's only clock.
const CHECKPOINT_CADENCE: Duration = Duration::from_mins(1);

/// One bounded saver: queued work is a workspace identity, never an engine
/// pin. It parks between opportunities rather than ticking: a queued parse
/// save rings its waker ([`Registry::saver_wakers`]), a deferred retry sets
/// its deadline ([`Registry::next_pending_parse_due`]), the checkpoint
/// cadence is the one clock, and the shutdown bell ends any park at once.
/// [`Registry::save_pending_parsed`] takes one due workspace per call, so
/// the loop goes straight around while any is still due.
///
/// # Errors
/// The waker could not be registered (a socketpair failed).
fn spawn_saver(registry: Arc<Registry>, shutdown: Arc<Doorbell>) -> io::Result<JoinHandle<()>> {
    let waker = registry.saver_wakers().register()?;
    Ok(thread::spawn(move || {
        let mut checkpoint_at = Instant::now();
        while !shutdown.is_rung() {
            waker.drain();
            registry.save_pending_parsed();
            if checkpoint_at.elapsed() >= CHECKPOINT_CADENCE {
                registry.save_observation_checkpoints();
                checkpoint_at = Instant::now();
            }
            let now = Instant::now();
            let checkpoint_in =
                CHECKPOINT_CADENCE.saturating_sub(now.duration_since(checkpoint_at));
            let timeout = registry
                .next_pending_parse_due()
                .map_or(checkpoint_in, |due| {
                    due.saturating_duration_since(now).min(checkpoint_in)
                });
            if let Err(e) = wake::wait_readable(&[shutdown.fd(), waker.fd()], Some(timeout)) {
                eprintln!("registry: saver wait: {e}");
                thread::sleep(timeout.min(wake::WAIT_FAILURE_BACKOFF));
            }
        }
    }))
}

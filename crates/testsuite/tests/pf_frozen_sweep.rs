...
//! survived earlier gates because frozen worked span/rev/root values lacked byte
//! pins; this module pins EVERY worked value the FROZEN `wire-contract-v2.md`
//! prints — enumerated in `results/pf-fixtures/frozen-value-enumeration.md` (137
//! occurrences) — as a fixture assertion recomputed by the engine, so a
//! wrong-grain value fails a test rather than shipping. Every value is derived
...
//!   Terminator-EXCLUDED (leaf block): `r-000042 [26,248]`, `r-000043
//!   [249,473]`, `fm_key [4,15]`. The file-length↔span-end off-by-one (249 vs
//!   `[26,248]`; 474 vs `[249,473]`) is asserted AS-IS, never normalized.
//! - **A7 — full-token root compare, prefix included.** The §12.3 v1 bump
//!   `b3a:83b4…8f68` is 64-hex-identical to R2 `b3:83b4…8f68`; a hex-tail-only
//!   compare FALSE-PASSES. Tokens asserted unequal AND hex-tails equal.
//! - **A3 — by-design equality named.** receipts `file_rev@S1` == receipts H1
//!   `node_rev` == `2731acfa39bbb92c` (the lone H1 spans the whole file), pinned
//!   deliberately to the SAME value so a distinctness lint cannot false-positive.
//! - **A2/A6 — single-sourced values re-derived, not copied.** Goals@S2
//!   `[20,150]`/`5a8faa717fbcdb04` (only printed in the §11.1 verdict) and the
//!   S2 resolve spans `[49,75]`/`[0,474]` are recomputed by build/walk here.

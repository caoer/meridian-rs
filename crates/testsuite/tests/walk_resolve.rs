...
/// parsed from `resolution.expected.json` (never hand-written). The pack records
/// native UTF-8 byte offsets (regenerated by the b3-pack-respan close), so each
/// span is consumed directly — no UTF-16 transcode.
fn oracle_answers(oracle: &Value) -> BTreeMap<String, Result<Location, Miss>> {
    ...
    for p in oracle["probes"].as_array().expect("probes array") {
        ...
        let result = if ans["ok"].as_bool() == Some(true) {
            ...
            Ok(Location {
                span: usize_at(&ans["span"][0])..usize_at(&ans["span"][1]),
                dest,
            })
        } else {
            let err = &ans["error"];
...
fn obsidian_compat_pack_green_through_walk() {
    ...
    let answers = oracle_answers(&oracle);
...
fn harness_p2_walk_probes_green_over_walkvault() {
    let (index, docs) = walkvault();
    let gt = oracle_answers(&oracle_json());
    let path =

...
The pin token is deliberately NOT a `span2` token on both entries: a git-folder
root's pin grain is the file, a vault root's is a parsed span, so the parser

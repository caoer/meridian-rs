//! U5.3 CLI wiring gate — drive the REAL `mrd` binary (`CARGO_BIN_EXE_mrd`) over
//! its process boundary for `mrd unfold` and `mrd new`: a marked workspace, a
//! preset def on disk, the operator-facing exit codes and birth receipts.

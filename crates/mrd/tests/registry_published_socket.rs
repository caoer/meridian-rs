...
impl Sandbox {
    ...
    fn run(&self, cwd: &Path, args: &[&str], stdin: Option<&str>) -> Output {
        self.run_with_daemon_bin(cwd, args, stdin, Path::new("/nonexistent/mrd-daemon"))
    }

    /// [`Self::run`] with the spawn target named: the default is spawn-impossible,
    /// and one gate points it at a child that models a lost flock race.
    fn run_with_daemon_bin(
        &self,
        cwd: &Path,
        args: &[&str],
        stdin: Option<&str>,
    ...
    /// A stand-in for `MERIDIAN_DAEMON_BIN` that models the LOSER's child in a
    /// concurrent cold start: it binds nothing (the flock is already held, so
    /// the real child would die here), and the only trace it leaves is the
    /// WINNER's publication, written after this client already looked and found
    /// none. Spawned as `<bin> daemon`; the argument is ignored.
    fn losing_daemon_bin(&self, winner: &Path) -> PathBuf {
        ...
        std::fs::write(
            ...
            format!(
                "#!/bin/sh\n\
                 # The winner bound {winner} and published it; this child dies on the flock.\n\
                 printf '%s\\n' '{winner}' > '{dir}/daemon.sock-path.tmp'\n\
                 mv '{dir}/daemon.sock-path.tmp' '{dir}/daemon.sock-path'\n\
                 exit 1\n",
                winner = winner.display(),
                dir = self.cache_root().join("registry").display(),
            ),
        )
...
/// The concurrent cold start, which the pre-spawn ping cannot cover: two
/// clients on different bases each find no derived socket AND no publication,
/// so both spawn. The winner binds its own base and publishes; the loser's
/// child dies on the flock. The fixture is that shape exactly — the lock
/// holder is live at a foreign base, its publication is taken away so this
/// client's pre-spawn ping finds nothing, and the child it spawns only writes
/// the winner's publication and exits nonzero.
///
/// The poll after a spawn therefore has to watch the publication as well as
/// the derived path. Watching the derived path alone spends the whole
/// `SPAWN_READY_TIMEOUT` and degrades with the lock holder live and published
/// the entire time — the measured defect in a narrower window, which is why
/// this gate asserts a WARM answer rather than a faster degrade.
#[test]
fn a_publication_that_appears_after_the_spawn_is_followed_before_the_timeout() {
    let sb = sandbox();
    let ws = sb.workspace();
    let daemon = sb.daemon();

    // The lock holder published at start; take it away, so this client's
    // pre-spawn ping has nothing to find and the spawn is reached.
    let publication = sb.cache_root().join("registry").join("daemon.sock-path");
    std::fs::remove_file(&publication).expect("the daemon published at start");
    assert!(
        !sb.derived_socket().exists(),
        "the fixture's premise: the child derives a socket nobody bound"
    );

    let bin = sb.losing_daemon_bin(&sb.published_socket());
    let started = std::time::Instant::now();
    let out = sb.run_with_daemon_bin(&ws, &["read", "doc.md"], None, &bin);
    let elapsed = started.elapsed();
    // Read BEFORE the shutdown, which removes the publication it wrote.
    let child_published = publication.exists();
    daemon.shutdown();

    assert!(
        child_published,
        "the fixture's own premise: the child wrote the winner's publication"
    );
    assert_served_warm(&out, "read after a lost flock race");
    assert!(
        elapsed < Duration::from_secs(4),
        "the ladder followed the publication rather than spending the spawn timeout: {elapsed:?}"
    );
}

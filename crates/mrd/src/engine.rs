...
/// auto-spawn a daemon detached and poll BOTH the DERIVED path — where the child, inheriting
/// this environment, binds — and the publication, which a concurrent cold start's winner may
/// write after the pre-spawn ping, until one answers or the timeout elapses, retargeting
/// `client` on success. Errors The daemon could not be spawned (spawn-impossible), or it was spawned but
...
pub(crate) fn ensure_daemon(client: &mut Client) -> io::Result<()> {
    ...
    while Instant::now() < deadline {
        ...
        // And the publication, EVERY tick, because the pre-spawn ping above could not have
        // seen it: two clients on different bases can each find no derived socket and no
        // publication and both spawn, and the loser's child dies on the flock while the
        // WINNER binds its own base and publishes. Polling only the derived path spends the
        // whole 5 s reaching a degrade with the lock holder live and published the entire
        // time — the measured defect this lane exists to close, in a narrower window. Re-read
        // rather than reuse `published`: the file is the winner's, written after that read.
        //
        // It costs one `read_to_string` of a small file per tick, and only on the cold lane
        // that has already paid a spawn and is waiting on a 5 s deadline — never on the warm
        // path, which returned at the first ping.
        if let Some(path) = registry::default_published_socket_path()
            && path.as_path() != derived.socket_path()
        {
            let holder = Client::new(path);
            if holder.ping().unwrap_or(false) {
                *client = holder;
                return Ok(());
            }

...
// ---------------------------------------------------------------------------
// SELF-RETIRING (U3.2 migration kit, d1 § Migration plan) — the `draws-from`→
// `inputs` frontmatter key-rename write op. This section DIES with the
// migration: its removal is in the pin leg's definition of done; the delete
// itself rides the U3.3 storm that consumes it (grep `SELF-RETIRING` to find
// every piece). Nothing on the parse/attest plane learns the dead key — the
// key text is spelled only by the caller (`crates/migrate`), never here.
// ---------------------------------------------------------------------------
//
// The migration renames one page's `draws-from:` frontmatter key to `inputs:`,
// value bytes preserved, under CAS write-what-you-read. Mechanically a
// COMPUTED-span splice (the grain is engine-computed from parse, never
// client-supplied) — the SAME shape as `pin_lock`, so the one-write-shape law
// holds. It journals ONE `op=migrate` wire audit row (`^r-NNNNNN`) and mints
// ZERO attestation receipts (`attest` is their sole minter, strict-writer law):
// the migration's trail is the journal row, never a receipt file.

/// One `migrate` key-rename request (d1 § Migration plan): a COMPUTED span edit
/// that replaces the `from_key` frontmatter grain of `path` with the same value
/// under `to_key`. `span`/`new_text` are engine-computed by the migrate kit;
/// `if_file_rev` is the page's whole-file rev the kit read (write-what-you-read
/// CAS); `before_rev`/`after_rev` are the `fm_key` grain's node-rev transition.
#[derive(Debug, Clone)]
pub struct MigrateKeyArgs {
    /// Frame correlation token — recorded only.
    pub id: Option<u64>,
    /// The page whose frontmatter key is renamed (workspace-confined).
    pub path: Path,
    pub actor: Option<String>,
    pub now: Option<String>,
    /// The optional §5.1 world guard: refuse if the ambient root differs.
    pub if_root: Option<Root>,
    /// The page's whole-file rev the kit computed against — write-what-you-read.
    pub if_file_rev: NodeRev,
    /// The renamed frontmatter key grain span (key line + any block value).
    pub span: Span,
    /// The renamed grain bytes (`from_key`→`to_key`, value byte-preserved).
    pub new_text: String,
    /// The dead key being retired (`draws-from`) — journal display only.
    pub from_key: String,
    /// The live key it becomes (`inputs`) — journal display only.
    pub to_key: String,
    /// The renamed grain's node rev BEFORE the write.
    pub before_rev: NodeRev,
    /// The renamed grain's node rev AFTER the write.
    pub after_rev: NodeRev,
    /// Dry run — everything except disk (no bytes, no journal row, no advance).
    pub dry: bool,
}

/// The outcome of a guarded `migrate` key-rename. Absences mirror
/// [`PinLockOutcome`]: `root_after`/`journal_anchor`/`committed` are `None` on a
/// dry run.
#[derive(Debug)]
pub struct MigrateKeyOutcome {
    pub root_before: Root,
    pub root_after: Option<Root>,
    /// The page's whole-file rev before the write (the CAS-confirmed rev).
    pub file_rev_before: NodeRev,
    /// The page's whole-file rev after the rename landed.
    pub file_rev_after: NodeRev,
    pub journal_anchor: Option<String>,
    pub committed: Option<DeltaFrame>,
    pub dry: bool,
...
/// **Guarded `migrate` key-rename** (d1 § Migration plan, U3.2): splice the
/// engine-computed renamed frontmatter grain into `path` under CAS
/// write-what-you-read + workspace-root, journal ONE `op=migrate` row, and emit
/// the `modified` change surface. Mints ZERO attestation receipts — the journal
/// row is the migration's sole trail (d1 § Actor on migration writes). Mirrors
/// [`pin_lock`] step for step so the one-write-shape law holds.
///
/// # Errors
/// `bad_path`, `bad_request` (reserved journal, or a bad computed span),
/// `root_mismatch` (stale world guard), `cas_mismatch` (the page drifted from
...
pub fn migrate_key(
    ...
    seq: u64,
    args: &MigrateKeyArgs,
) -> Result<MigrateKeyOutcome, Box<ErrorBody>> {
    let fs_path = FsPath::new(&args.path.0);
    path_confined(&args.path)?;
    reserved_journal_guard(fs_path)?;
    ...
    // write-what-you-read CAS: the page must still carry the rev the kit read
    // (a drift means the grain it recomputed the rename over is stale bytes).
    if args.if_file_rev != file_rev_before {
        return Err(cas_mismatch(&args.if_file_rev, &file_rev_before));
    }
    ...
    {
        return Err(bad_request(
            "migrate key span is out of bounds or splits a character",
        ));
    }
    let mut new_raw = String::with_capacity(raw.len() + args.new_text.len());
    new_raw.push_str(&raw[..start]);
    ...
    let journal_anchor = migrate_journal_write(root, args, &root_before, &root_after)?;

    Ok(MigrateKeyOutcome {
        root_before,
        root_after: Some(root_after),
        file_rev_before,
        file_rev_after,
...
/// Journal one guarded `migrate` (SELF-RETIRING, U3.2): an `op=migrate` row with
/// a single `match` edit naming the renamed `from_key` (its grain before→after
/// rev). Carries BOTH roots (chain continuity) and appends to the reserved
/// root-EXCLUDED journal via the receipt-engine append. No attestation receipt
/// is written — the row is the migration's sole trail. Returns the row anchor.
fn migrate_journal_write(
    root: &fs::WorkspaceRoot,
    args: &MigrateKeyArgs,
    root_before: &Root,
    root_after: &Root,
    ...
    let seq = next_journal_seq(root)?;
    let target = SecRef::FmKey {
        fm_key: args.from_key.clone(),
    };
    let shape = EditShape::Match {
        old: args.from_key.clone(),
        new: args.to_key.clone(),
    };
    let line = receipt::journal::render_row(&receipt::journal::JournalRow {
        seq,
        op: "migrate",

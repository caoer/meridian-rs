...
Run 2026-07-28 19:20 UTC · `zmax` (aarch64 macos, Apple M4 Max, 16 cores) · git `6c0f10da` · rustc 1.97.1 (8bab26f4f 2026-07-14) · report schema v1
...
| `assemble.p99.file` | us | — | — | 111.871 | MEASURED | LIVE (model::build landed 730c37b7/M2-BUILD): hdr p99 of one build over the parsed corpus, parse+clone outside the timed window. Metric is us because the hdr path stages percentile_us and the join is unit-blind. No baseline yet — the first fleet run establishes it |
...
| `transport.codec.ndjson_roundtrip_p99` | us | — | — | 29.087 | MEASURED | LIVE day 1 (NdjsonCodec is implemented); no baseline yet — first fleet run establishes it |
| `transport.codec.proto_roundtrip_p99` | us | — | — | 5.335 | MEASURED | LIVE (typed protobuf path, ZT ruling 2026-07-18); same frame mix as the ndjson claim for codec-to-codec comparison; no baseline yet — first fleet run establishes it |
| `ingest.cold.vault_1gb` | ms | 22200.000 | ≤ 2220000 | 47597.560 | PASS | walk+read today; syntax::parse joins the pass at rung 1 |
| `ingest.codec.ndjson.vault_1gb` | ms | 1780.000 | ≤ 178000 | 2324.848 | PASS | the untyped seam's real bulk cost: JSON string escaping both ways. Frames pre-built outside the timed window |
| `ingest.codec.proto.vault_1gb` | ms | 583.000 | ≤ 58300 | 660.305 | PASS | the typed path's real bulk cost on identical content — codec-to-codec comparison against the ndjson claim |
...
| `policy.evaluate.p99.vault_1gb` | us | — | ruleset | 69.119 | MEASURED | P6-EVAL wall-time: p99 of one policy::evaluate call over a real model::build AST (blurb-required Starlark pack). Measured not asserted (risk R2 posture); gate arrives from policy vocab() Budget{p99_us} when it lands — ruleset-sourced so a local number never falsely PASS/FAILs |
| `policy.evaluate.p99.fence_bomb` | us | — | ruleset | 66.175 | MEASURED | P6-EVAL wall-time on the adversarial fence-saturated shape: p99 of one policy::evaluate call over a real AST. Measured not asserted (risk R2); ruleset-sourced gate pending vocab() |
...
|---|---|---|---|---|---|---|---|---|
| `assemble.p99.file` | 111.871 | us | 8000 | 16.7 | 33.6 | 111.9 | 273.7 | 2332.7 |
| `policy.evaluate.p99.fence_bomb` | 66.175 | us | 8000 | 34.3 | 37.5 | 66.2 | 132.7 | 216.2 |
| `policy.evaluate.p99.vault_1gb` | 69.119 | us | 8000 | 25.2 | 28.9 | 69.1 | 165.1 | 249.6 |
| `policy.splice.match_scan.p99` | 342.271 | us | 8000 | 106.9 | 178.7 | 342.3 | 730.6 | 3119.1 |
| `transport.codec.ndjson_roundtrip_p99` | 29.087 | us | 20000 | 20.9 | 25.2 | 29.1 | 73.0 | 125.4 |
| `transport.codec.proto_roundtrip_p99` | 5.335 | us | 20000 | 4.4 | 5.0 | 5.3 | 20.5 | 52.9 |

## Criterion estimates

| bench | mean | median |
|---|---|---|
| `assemble_build_vault2026` | 18.77 µs | 17.85 µs |
| `parse/placeholder_corpus_traversal` | 90 ns | 83 ns |
| `policy_evaluate_vault1gb_batch` | 7.09 ms | 6.33 ms |
| `roundtrip/ndjson_decode_1000` | 11.94 ms | 11.36 ms |
| `roundtrip/ndjson_encode_1000` | 1.75 ms | 1.62 ms |
| `roundtrip/proto_decode_1000` | 3.01 ms | 2.22 ms |
| `roundtrip/proto_encode_1000` | 865.18 µs | 794.42 µs |
